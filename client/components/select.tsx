...
export function Select({ label, options = [] }: PropsWithChildren<{
    label?: string
    ...
    return (
        <Listbox value={selected} onChange={setSelected}>
            {({ open }) => (
                <>
                    {label ? <Listbox.Label className="block text-sm font-medium text-white mb-1">
                        {label}
                    </Listbox.Label> : null}
                    <div className="relative">
                        <Listbox.Button className="relative w-full bg-dark-700 border border-transparent rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-primary-500 focus:border-primary-500 sm:text-sm">
                            <span className="flex items-center">
                                {selected?.image ? <img src={selected.image} alt={selected.name} className="flex-shrink-0 h-6 w-6 rounded-full" /> : null}
                                        ...
                                        {({ selected, active }) => (
                                            <>
                                                <div className="flex items-center">
                                                    <img src={option.image} alt="" className="flex-shrink-0 h-6 w-6 rounded-full" />
                                                    <span
