...
export function JoinCommunity({
  ...
  const { isLoading: isJoining, mutate: joinCommunity } = useMutation(
    async () => {
      await instance.post(`/communities/${community.id}/memberships`)
  ...
  return (
    ...
    >
      ...
      <div className="mt-3 text-center text-sm max-w-[24rem] mx-auto">{community.description}</div>

      <PrimaryButton
        isLoading={isJoining}
        className="px-12 w-full mt-12 mb-4"
...
export function CreateCommunity({
  ...
  const { isLoading: isLoadingBadges, data: badges = [] } = useQuery<Badge[]>(
    ['badges'],
    async () => {
  ...
  const { isLoading: isCreatingCommunity, mutate: createCommunity } = useMutation(
    async () => {
  ...
  return (
    <Modal
      ...
      setIsOpen={setIsOpen}
      hideHeading
      positionClasses="fixed inset-x-0 overflow-y-auto top-0 lg:mt-6"
      size="large"
    >
      <div
        className="-mx-6 h-24 lg:h-36 bg-red-500 -mt-6 cursor-pointer"
        role={'button'}
        onClick={console.log}
      ></div>

      <img
        src="https://res.cloudinary.com/bahdcoder/image/upload/v1660309953/avatars/Cranks-1.png"
        className="w-32 h-32 -mt-16 mx-auto"
        alt="community logo"
      />
      <div className="py-6">
        <Input
          label="Community name"
          placeholder="Degen Traders"
          onChange={(event: any) => {
        ...
        <div className="mt-6">
          ...
          <ReactTextareaAutosize
            ...
            className={classNames(
              'rounded-lg text-sm p-4 border  bg-transparent min-h-[4rem] lg:min-h-[5rem] text-white focus:outline-none w-full text-md resize-none placeholder:text-dark-300 focus:ring-1',
              {
                'focus:ring-primary-500 focus:border-primary-500 border-dark-400':
                  !errors.description,
          ...
          {errors?.description ? (
            ...
            <small className="text-xs text-dark-300">Provide the purpose of this community.</small>
          )}
        </div>
        <div className="mt-6">
          ...
          />
          {errors?.whitelistedTokens ? (
            <small className="text-xs text-red-500">{errors.whitelistedTokens}</small>
          ) : (
            <small className="text-xs text-dark-300">
              Provide a comma separated list of all whitelisted tokens. Anyone holding any of these
              tokens will gain access to your community.
            </small>
          )}
        </div>
        ...
        <div className="mt-6">
          ...
          {isLoadingBadges ? (
            ...
            <Select
              ...
              options={badges?.map((badge) => ({ name: badge.title, id: badge.id }))}
              onChange={(option) => {
          ...
          <small className="text-xs text-dark-300">
            Which Web3 journey best matches this community ?
          </small>
        </div>
      </div>
...
export function SingleCommunity({
  community,
}: PropsWithChildren<{
  community: Community
}>) {
  ...
  return (
    <div className="rounded-2xl bg-black border border-dark-400 ">
      ...
      <div className="flex flex-col items-center px-6 pb-8 rounded-b-lg">
        ...
        <div className="mt-3 text-sm text-center">{community.description}</div>
...
export default function Communities() {
  ...
  const { data: communities, isLoading: isFetchingCommunities } = useQuery<{ data: Community[] }>(
    ['communities'],
    async function () {
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-0">
          <div className="flex flex-col lg:flex-row lg:justify-between">
            <div className="flex flex-col">
              <h1 className="text-4xl font-bold font-grotesk text-white">Web3 Communities</h1>
              <p className="mt-3 text-md">
                Discover communities to help you grow your career in Web3
              </p>
            </div>

            <div className="mt-5 lg:mt-0">
              <PrimaryButton className="px-10 py-4" onClick={() => setShowCreateModal(true)}>
                Create community
              </PrimaryButton>
            </div>
          </div>
      ...
      <div className="max-w-7xl mx-auto mt-16 px-4 lg:px-0">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 mb-64">
          {isFetchingCommunities
            ? [1, 2, 3, 4, 5, 6].map((x) => <CommunitySkeleton key={x} />)
            : null}
